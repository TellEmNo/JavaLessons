...
public class View {
    ...
    private static String getChar(int x, int y){
        String out = "| ";
        for (BaseHero human: Main.battle) {
    ...
    public static void view() {
        if (step == 0 ){
            System.out.print(AnsiColors.ANSI_YELLOW + "Starting positions" + AnsiColors.ANSI_RESET);
        } else {
            System.out.print(AnsiColors.ANSI_YELLOW+ "Step " + step + AnsiColors.ANSI_RESET);
        }
        step++;
        Main.battle.forEach((v) -> l[0] = Math.max(l[0], v.getInfo().length()));
